...
/**
 * ArticleController implements the CRUD actions for Artikel model.
 */
class ArticleController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                $imageName_ = $model->kategori;
                if($model->file = UploadedFile::getInstance($model,'file')){ 
                    $model->foto= '' .$model->file->extension;
                    $model->save();
                    $imageName = $model->id;
                    $model->file->saveAs('uploads/'.$imageName_. '-'.$imageName.'.'.$model->file->extension);
    ...
    {
        if (($model = Artikel::findOne(['id' => $id])) !== null) {
            return $model;
        }
